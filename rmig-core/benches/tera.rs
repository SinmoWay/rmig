...
fn simple_apply_context_and_resolve_vars(pattern: &str, env: HashMap::<String, String>) -> anyhow::Result<()> {
...
fn tera_apply_context_and_resolve(c: &mut Criterion) {
    c.bench_function("Testing tera resolving.", |b| b.iter(|| {
        let mut env = HashMap::<String, String>::new();
        env.insert(String::from("name"), String::from("WORLD"));
        env.insert(String::from("table"), String::from("DUAL"));
        simple_apply_context_and_resolve_vars("SELECT {{ name }} FROM {{ table }};", env).unwrap();
    }));
}
